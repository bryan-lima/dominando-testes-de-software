# Dominando Testes de Software | .NET 6

Projeto de estudos para praticar conteúdo do curso **[Dominando os Testes de Software](https://desenvolvedor.io/curso-online-dominando-os-testes-de-software)** da plataforma **[desenvolvedor.io](https://desenvolvedor.io)**

## Execução de testes via CLI

- Instalar pacote `xunit.runner.console`
- Após, realizar publish da aplicação (build)
- Na pasta do publish, abrir o terminal e executar o comando:

```cmd
dotnet vstest NomeProjeto.Tests.dll
```
<h1 align="center"> 
	<img alt="Exemplo de execução de testes via CLI" title="Exemplo de execução de testes via CLI"  src="https://raw.githubusercontent.com/bryan-lima/dominando-testes-de-software/master/.github/ExemploExecucaoTestesViaCLI.png" width="100%">
</h1>